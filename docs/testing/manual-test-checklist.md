# FusionXPay Manual Test Checklist (Template)

## Scope

- Real-environment manual validation for core payment flows, admin operations, RBAC isolation, and auth boundaries.
- Use this checklist after deploying changes to a target environment.

## Execution Metadata

- Environment: `<APP_HOST> (app host) / <MIDDLEWARE_HOST> (middleware host)`
- Base URL: `<BASE_URL>` (e.g. `http://<APP_HOST>:8080`)
- Build / Commit SHA: `<fill>`
- Tester: `<fill>`
- Execution Date: `<fill>`
...
- CORS config includes deployed frontend domain and localhost origins.
